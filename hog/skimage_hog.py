...
class HOG_Feature:
    ...
    def vector(self, img, rsize=True):
        image = cv2.imread('images/' + img,0)
        if rsize == True: image = resize(image, (96, 128), anti_aliasing=False)
        feature_vector = hog(image, orientations=self.bins, pixels_per_cell=(self.cell, self.cell),
            cells_per_block=(self.block,self.block), block_norm="L2", feature_vector=True)
        return feature_vector

    def visualize(self, img, rsize=True):
        image = cv2.imread('../images/' + img,0)
